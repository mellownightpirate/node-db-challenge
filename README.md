...
SQL databases are primarily called as Relational Databases (using the relational model of data); whereas NoSQL database are primarily called non-relational or distributed database. SQL is the language (set of words) used to query such data. A Relational Database is when we store data in table formats; where normalisation should be followed to ensure effective structuring of data.

- [x] Why do tables need a `primary key`?

To make each row unique and identifiable
...
A many-to-many relationship between tables means one table may contain a parent for which there are many children in the other table and vice versa.
